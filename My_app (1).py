...
# 1. ページの設定
st.set_page_config(
    page_title="フォークソング研究部機材管理表", page_icon="🎸", layout="wide"
)

st.title("🎸 フォークソング研究部 機材管理表")
st.markdown("---")

# 2. データの初期化・読み込み（CSVファイルから読み込む）
if "equipment_data" not in st.session_state:
    if os.path.exists(DATA_FILE):
        # 保存されたCSVがあればそれを読み込む
        st.session_state.equipment_data = pd.read_csv(
            DATA_FILE, dtype={"機材ID": str}
        )
    else:
        # なければ初期データを読み込んでCSVとして保存する
        initial_df = get_initial_data()
        initial_df.to_csv(DATA_FILE, index=False, encoding="utf-8")
        st.session_state.equipment_data = initial_df
        ...
        with st.form("edit_form", clear_on_submit=False):
            ...
            with date_col1:
                ...
                selected_year = st.selectbox(
                    "年",
                    options=year_list,
                    index=year_list.index(curr_year),
                    key="edit_y",
                )
            with date_col2:
                ...
                selected_month = st.selectbox(
                    "月",
                    options=month_list,
                    index=month_list.index(curr_month),
                    key="edit_m",
                )
            with date_col3:
            ...
            if submit_button:
                ...
                st.session_state.equipment_data.at[
                    selected_index, "現在の状態"
                ] = new_status
                st.session_state.equipment_data.at[
                    selected_index, "前回メンテ日"
                ] = new_date_str

                # 💡 変更したデータをCSVファイルに書き込んで永続化
                st.session_state.equipment_data.to_csv(
        ...
        with st.form("add_form", clear_on_submit=True):
            ...
            with date_col1:
                ...
                add_year = st.selectbox(
                    "年", options=year_list, index=2, key="add_y"
                )  # デフォルト2026年
            with date_col2:
                month_list = [f"{m:02d}" for m in range(1, 13)]
                add_month = st.selectbox(
                    "月", options=month_list, index=4, key="add_m"
                )  # デフォルト05月
            with date_col3:
                day_list = [f"{d:02d}" for d in range(1, 32)]
                add_day = st.selectbox(
                    "日", options=day_list, index=21, key="add_d"
                )  # デフォルト22日
            ...
            if add_button:
                if not add_id or not add_name:
                    ...
                    st.error(
                        f"❌ 機材ID「{add_id}」は既に使われています！別のIDにしてください。"
                    )
                else:
